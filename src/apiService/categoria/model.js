...
const Categoria = function (categoria) {
    ...
    this.ca_u_create = categoria.ca_u_create,
    this.ca_u_update = categoria.ca_u_update,
...
Categoria.findByName = (ca_nombre, result) => {
    ...
    sql.query(`select count(*) as result from categoria where ca_nombre = ?`, ca_nombre, (err, res) => {
...
Categoria.findById = (id, result) => {
    ...
    sql.query(`SELECT * FROM categoria WHERE ca_id = ${id} and eliminado = 0`, (err, res) => {
...
Categoria.updateById = (id, categoria, result) => {
    ...
    sql.query("UPDATE categoria SET ca_nombre = ?, ca_f_update = NOW(), ca_u_update = ? , eliminado = ? WHERE ca_id = ?",
...
Categoria.remove = (id, result) => {
    ...
    sql.query("UPDATE categoria SET eliminado = 1 WHERE ca_id = ?", id, (err, res) => {
